...
EXPANSION: list[Spec] = [
    ...
    Spec("ME.9", 2, "He remembers what he hears, says, and does — attributed",
         ...
         notes="COVERS: memory across lives (claim), social/other agents "
               "(claim)\n"
               "KIND DECLARED by the Review 2026-09-13 (anatomy audit). This "
               "spec was credited as a challenger with no COVERS kind the "
               "moment the `Person model` seat named it, which broke the "
               "KINDLESS-DISCHARGES ratchet at 1. Declared rather than "
               "un-named: the seat is about whose advice proved true, and "
               "attributed recall is exactly the capability that could unseat "
               "a person-model that turned out to be text similarity — which "
               "is what the 08-30 strengthening below found it had been. "
               "`social/other agents` is added to the domain because "
               "attribution per SPEAKER is a claim about people and not only "
               "about storage.\n"
               "STRENGTHENED by the Review 2026-08-30 (Part 2), three gates "
    ...
    Spec("D1.0", 2, "Control-path bakeoff: who does motor control (D1's four "
         ...
         hypothesis="Among the four permitted control-path architectures — "
                    ...
                    "the 3-sigma learning gate scored PAIRED against its own "
                    "untrained twin (ADOPTED GATE 2026-09-06, replacing 'vs "
                    "the random null': mean(d)/(sd(d)/sqrt(n)) >= 3.0 with "
                    "d_i = trained_i - untrained_i per shared seed, bar "
                    "unmoved) AND holds seed consistency sd/|mean| <= 0.50 "
                    "(else INCONSISTENT, a distinct verdict), and one "
         ...
         control="Untrained twins of ALL FOUR arms must miss the learning "
                 ...
                 "PLUS the G3 external reference (adopted gate 2026-09-06): "
                 "T2.02's verbatim SB3 PPO MLP recipe trains to STEP_TARGET "
                 "in its own kernel FIRST; below 450 mean return the run is "
                 "VOID as a HARNESS fault, no learning verdict on any arm, "
                 "and the arm kernels are never submitted.",
         ...
         notes="Registered by D1's firing commit 2026-09-01 per the executor "
               ...
               "PLUS a ~1.5 h reference kernel dispatched first (G3). "
               "Module-cache the kernel (one-submission-per-spec rule). "
               "ADOPTED GATE 2026-09-06 (Review FULL, both d10-* queue "
               "rows): G1 paired-own-twin learning statistic, G2 "
               "consistency conjunct, G3 SB3 reference floor — all "
               "strengthen-only, sigma bar unmoved; attempt 1's VOID "
               "stands (T2.02 precedent); full record in the test "
               "docstring's ADOPTED GATE block.",),
    ...
    Spec("PS.06", 2, "Tiring is a price: sustained exertion drains the body's "
                     "capability before anyone has to learn it",
         hypothesis="W0's needs layer charges for work in a currency the body "
                    "must repay: a scripted sustained-exertion policy drives "
                    "fatigue f (needs.py:528 — f rises with mechanical power "
                    "p_mean, falls with rest, taus 60 s/60 s so steady-state "
                    "f* = P/p_max) high enough on every seed that the "
                    "weakness gate gear_scale = 0.5 + 0.5*(1-f)*min(e,i) "
                    "(needs.py:382, GEAR_FLOOR 0.5) measurably cuts what the "
                    "SAME ctrl commands achieve — the spent-vs-fresh "
                    "capability gap, read as achieved mechanical output under "
                    "identical ctrl, exceeds the venue's outcome-metric "
                    "quantum (the DP.04/W1.02 resolution lesson applied at "
                    "registration); rest REPAYS it at the pre-registered "
                    "TAU_F_FALL timescale, distinguishing tiredness from "
                    "damage; and the imminent droop is LEGIBLE beforehand — "
                    "a probe on the sensory vector (the NEED_DIM block "
                    "carries f) predicts the next-window capability deficit "
                    "well above chance while the body is still fresh enough "
                    "to act on it (PS.02's legibility gate, reused because "
                    "it is the same question).",
         falsified_by="At the body's actually-sustainable power fraction, f "
                      "never accrues enough to move gear_scale (tiredness is "
                      "decorative at W0's rates — f* = P/p_max may sit near "
                      "0 for a body that cannot sustain power); or the "
                      "spent-vs-fresh gap reads below the outcome quantum, "
                      "so no learner could ever be selected on pacing; or "
                      "rest does not restore capability on the registered "
                      "timescale (then the price is damage wearing fatigue's "
                      "name, already covered by PS.03); or the probe cannot "
                      "beat chance, so tiring is a price paid blind. Any of "
                      "the four means 'tiring' is not learnable in this "
                      "venue and GOAL.md:187's promise has no mechanism "
                      "behind it.",
         null_baseline="A FATIGUE-FROZEN twin: identical world, seeds and "
                       "exertion schedule with f clamped at 0 — its "
                       "capability curve must stay FLAT. A droop that "
                       "survives the freeze is energy drain, damage or a "
                       "clock artifact, not tiredness (gear_scale couples "
                       "min(e,i) too; the freeze isolates f's share). And a "
                       "shuffled probe pairing, at chance.",
         metric="spent_vs_fresh_capability_gap_vs_quantum",
         budget=Budget.CPU, seeds=3, depends_on=["NE.00"],
         control="THE SENSORY AMPUTATION (PS.02's control, reused): the same "
                 "exertion schedule with the interoceptive block removed "
                 "from the probe's input — the probe must FAIL there, or it "
                 "was reading the episode clock or the already-visible "
                 "kinematic droop rather than a sense. RIG GATE, "
                 "pre-registered: the scripted policy must SUSTAIN a "
                 "pre-registered fraction of p_max on every registered seed "
                 "(alive-proof pinned to measured p_mean, never locomotion "
                 "— this body tips over in seconds and thrash counts as "
                 "work); a body that cannot exert VOIDs the run on the rig "
                 "and records a BODY finding, never a world verdict.",
         kills="On FAIL: 'tiring' is struck from what W0 can teach, every "
               "pacing, effort-budgeting or rest-scheduling claim is "
               "inadmissible in this venue, and the repair is a world/body "
               "edit routed through the W1 window or NE.01's constants "
               "lane under ITS binding constraints — never a quiet "
               "TAU_F/GEAR_FLOOR retune until tiredness appears.",
         notes="COVERS: tiring (claim).\n"
               "Registered under the 102nd audit RANK 1 / FOR THE BUILDER "
               "item 1 (2026-09-19), sibling 2 of 3 after PS.05: "
               "GOAL.md:187 names seven survival-earned primitives and "
               "'tiring' had NO spec at all — not blocked, not parked, "
               "absent, invisible to every id-ranking instrument. Shape "
               "stolen from PS.05 deliberately: a world-prices-it claim "
               "needing NO trained champion (scripted exertion + probe), so "
               "it does not queue behind the learning-core redesigns. "
               "Namespace: PURPOSE_AND_SCAFFOLDING.md's historical PS.06 "
               "question (drive decomposition) was DECIDED in "
               "NEEDS_AND_DEATH.md (its need-x-ablation row marks 'PS.06's "
               "question, now decided here'), so the id is free — the same "
               "way the retired scaffold-removal concept freed PS.05's. "
               "depends_on NE.00 (the algebra certificate), NOT NE.01: "
               "NE.01's FAIL is about survival-by-accident and is "
               "orthogonal to whether exertion is priced; disclosed here so "
               "the choice is auditable. The learned half — a trained agent "
               "PACES itself, resting before it is spent — is a SIBLING to "
               "register when a learner exists; do not fold it in here (the "
               "ME.11 per-partition lesson). The exertion schedule, the "
               "sustainable-power fraction for the alive-proof, the "
               "capability metric and its quantum source, and the probe "
               "target/window are pre-registered in the test docstring "
               "before its first run (T3.06 extreme-value lesson), and the "
               "fatigue-frozen twin plus the amputated probe are the two "
               "controls that MUST fail."),

    Spec("PS.08", 2, "Heavy is a price: moving mass costs need-currency "
                     "before anyone has to learn it",
         hypothesis="The venue charges for moving mass, in the currency the "
                    "needs already speak: a scripted transport policy driven "
                    "to displace playground objects of >= 4 pre-registered "
                    "masses (inside the EXISTING object_mass_range 0.2-3.0 "
                    "kg, playground.py:123 — no new world content) over an "
                    "identical displacement pays a need-cost (integrated "
                    "drive drain read off the existing needs.py channels: "
                    "M = M_BASAL + kappa_act*p_mech, needs.py:245, with "
                    "fatigue riding the same p_mean, needs.py:513 — no new "
                    "instrument) that increases MONOTONICALLY in object "
                    "mass on every seed; the light-vs-heavy cost gap "
                    "exceeds the venue's outcome-metric quantum (the "
                    "DP.04/W1.02 resolution lesson applied at registration, "
                    "not discovered by a pilot); and the load is LEGIBLE "
                    "beforehand — a probe on the sensory vector predicts "
                    "the trip's coming cost (or the mass class) above "
                    "chance early in the push, while there is still time "
                    "to decline the load (PS.02's legibility gate, reused "
                    "because it is the same question).",
         falsified_by="Cost flat or non-monotone in mass (moving mass is "
                      "effectively free — friction and inertia never reach "
                      "the needs accounting at this body's scales); or the "
                      "light-vs-heavy gap reads below the outcome quantum, "
                      "so no learner could ever be selected on load; or "
                      "the probe cannot beat chance, so heavy is a price "
                      "paid blind. Any of the three means 'heavy' is not "
                      "learnable in this venue and GOAL.md:187's promise "
                      "has no mechanism behind it.",
         null_baseline="An EQUALISED-MASS twin: the same push schedule "
                       "with every object set to the lightest registered "
                       "mass — its cost-vs-NOMINAL-mass curve must be "
                       "FLAT. A monotone cost that survives "
                       "mass-equalisation is a schedule or venue artifact, "
                       "not a price of mass. And a shuffled probe pairing, "
                       "at chance.",
         ...
         control="THE SENSORY AMPUTATION (PS.02's control, reused), "
                 ...
                 "measured this rover TOPPLING under sustained push, so "
                 "the gate must be earned, never assumed); a body that "
                 "cannot move the heaviest object VOIDs the run on the "
                 "rig and records a BODY finding, never a world verdict. "
                 "TAUTOLOGY GUARD, also rig: p_mech must be MEASURED from "
                 "the physics (actuator work done against the object), "
                 "never synthesised from the nominal mass — a fixture "
                 "that computes work as mu*m*g*d has assumed the "
                 "conclusion and cannot test it.",
         kills="On FAIL: 'heavy' is struck from what this venue can "
               "teach, every carrying, hauling or load-budgeting claim is "
               "inadmissible here, and the repair is a world/body edit "
               "routed through the W1 window — never a quiet friction or "
               "mass-range retune until the price appears.",
         notes="COVERS: heavy (claim).\n"
               "Registered under the 103rd audit FOR THE BUILDER item 1 "
               "(2026-09-19), sibling 3 of 4 after PS.05/PS.06: "
               "GOAL.md:187 names seven survival-earned primitives and "
               "'heavy' had NO spec at all — not blocked, not parked, "
               "absent, invisible to every id-ranking instrument. Shape "
               "stolen from PS.05/PS.06 deliberately: a world-prices-it "
               "claim needing NO trained champion (scripted pushes + "
               "probe), so it does not queue behind the learning-core "
               "redesigns. PG.2's nomination is NOT declared — it is "
               "buoyancy/density, a keyword artifact (the 102nd and 103rd "
               "audits both name the trap). Namespace: PS.07 is HELD for "
               "the W1 needs-off transfer battery (DIRECTION_AUDIT §5 "
               "disposition above) and is skipped deliberately; PS.08 is "
               "verified free in the registry, the ledger and "
               "PURPOSE_AND_SCAFFOLDING.md. depends_on NE.00 (the needs "
               "algebra certificate, the accounting this claim reads), "
               "deliberately NOT a locomotion spec: the body's ability to "
               "push is the RIG GATE's burden and an honest VOID lane, "
               "not a dependency — disclosed here so the choice is "
               "auditable. The learned half — a trained agent prefers "
               "light loads at equal value, or abandons what is too "
               "heavy — is a SIBLING to register when a learner exists; "
               "do not fold it in here (the ME.11 per-partition lesson). "
               "Masses, the displacement, the push schedule, the cost "
               "accounting, the quantum source and the probe "
               "target/window are pre-registered in the test docstring "
               "before its first run (T3.06 extreme-value lesson), and "
               "the equalised-mass twin plus the amputated probe are the "
               "two controls that MUST fail."),

    Spec("PS.09", 2, "Worth-it is a real distinction: the world pays and "
                     "charges in one currency, and some trips do not pay",
         hypothesis="W0's ledger makes net value computable and "
                    "NON-VACUOUS: resource payoffs and traversal prices "
                    "are the SAME need-currency (floor food restores nu = "
                    "0.08, drives.py:68, while PS.05 MEASURED the "
                    "traversal price at 0.016-0.069 over 1.5-6 m — the "
                    "payoff sits INSIDE the price span, so a break-even "
                    "exists in-world at registration time), and across "
                    ">= 4 pre-registered (payoff, distance) offers "
                    "straddling that break-even, a scripted collect-trip's "
                    "NET need-gain (restored minus drained, both read off "
                    "the existing needs.py channels — no new instrument) "
                    "is POSITIVE for at least one registered offer and "
                    "NEGATIVE for at least one, on every seed, each "
                    "side's margin exceeding the venue's outcome-metric "
                    "quantum; and the sign is LEGIBLE beforehand — a "
                    "probe on the PRE-DEPARTURE sensory vector predicts "
                    "the trip's net sign above chance before the first "
                    "step (PS.02's legibility gate, reused because it is "
                    "the same question).",
         falsified_by="Every reachable offer is net-positive (there is "
                      "nothing to decline, so 'worth-it' is a distinction "
                      "the world never asks for); or every offer is "
                      "net-negative (nothing is worth doing and foraging "
                      "is a losing game at every distance); or the "
                      "margins at the straddle read below the outcome "
                      "quantum, so no learner could ever be selected on "
                      "declining; or the probe cannot beat chance, so "
                      "worth-it is decided blind. Any of the four means "
                      "'worth-it' is not learnable in this venue and "
                      "GOAL.md:187's promise has no mechanism behind it.",
         null_baseline="A FREE-LUNCH twin: the same offers with traversal "
                       "made instantaneous (PS.05's teleport twin, "
                       "reused) — under it the negative branch must "
                       "VANISH (net = payoff minus ~nothing), because a "
                       "net-negative that survives free traversal is a "
                       "payoff-accounting artifact, not a price-vs-payoff "
                       "trade. And a shuffled probe pairing, at chance.",
         metric="net_gain_sign_split_and_margins_vs_quantum",
         budget=Budget.CPU, seeds=3, depends_on=["PS.01"],
         control="THE SENSORY AMPUTATION (PS.02's control, reused): the "
                 "same offers with the offer-bearing channels removed "
                 "from the probe's input — the probe must FAIL there, or "
                 "it was reading a schedule artifact rather than a sense. "
                 "RIG GATE, pre-registered: the scripted policy must "
                 "actually REACH and EAT on every registered offer and "
                 "seed (alive-proof — eating is mouth-gated, needs.py:29; "
                 "PS.05's fixture proved reach to 6 m, and any longer "
                 "registered distance re-earns it here); a body that "
                 "cannot collect VOIDs the run on the rig and records a "
                 "BODY finding, never a world verdict.",
         kills="On FAIL: 'worth-it' is struck from what W0 can teach, "
               "every cost-benefit, declining or foraging-selectivity "
               "claim is inadmissible in this venue, and the repair is a "
               "world edit routed through the W1 window (a payoff "
               "schedule and a price curve that actually cross) — never "
               "a quiet nu or decay-rate retune until the trade appears.",
         notes="COVERS: worth-it (claim).\n"
               "Registered under the 103rd audit FOR THE BUILDER item 1 "
               "(2026-09-19), sibling 4 of 4, closing the GOAL.md:187 "
               "series after PS.05 (far), PS.06 (tiring) and PS.08 "
               "(heavy). The audit said this one 'will need the venue "
               "described before it can be phrased' — the venue IS "
               "PS.05's fixture plus the payoff the world already "
               "commits to: floor food restores 0.08 while the measured "
               "traversal price spans 0.016-0.069 over 1.5-6 m, so both "
               "signs of trade plausibly exist at or just beyond the "
               "proven envelope (longer offers re-earn reach at the rig "
               "gate), and the claim is that they DO — falsifiable in "
               "both directions by arithmetic the world has already "
               "published. depends_on PS.01 (the drive layer is a real "
               "control problem), deliberately NOT PS.05: PS.05's FAIL "
               "is one legibility conjunct on a band-lottery estimator "
               "(routed to the Review, DUE 09-24), orthogonal to whether "
               "prices exist — its world half is green on every seed — "
               "and welding this registration behind that repair would "
               "hold a free action hostage to a ruling nobody needs for "
               "it (the goal-187 fork lesson) — disclosed here so the "
               "choice is auditable. The learned half — a trained "
               "forager DECLINES net-negative offers — is a SIBLING to "
               "register when a learner exists; do not fold it in here "
               "(the ME.11 per-partition lesson). Offers, the break-even "
               "straddle, the payoff/price accounting, the quantum "
               "source and the probe target/window are pre-registered in "
               "the test docstring before its first run (T3.06 "
               "extreme-value lesson), and the free-lunch twin plus the "
               "amputated probe are the two controls that MUST fail."),
    ...
    Spec("W1.00", 2, "The null is the strongest process that has not learned",
         hypothesis="For a W-venue claim spec, the honest null is the "
                    "best-scoring member of {stationary white, "
                    "temporally-correlated colored noise at the W0.DIAG "
                    "schedule, repeat-action} SELECTED ON THE NULL'S OWN "
                    "OUTCOME, never on the claim arm's — and re-scoring the "
                    "Pile A shallowness findings (LC.03's darkroom control, "
                    "LC.03 v2's one-learner-in-five, field-watch wk5's "
                    "random-covers-W0, T3.06's random_dwell_worst_life) "
                    "against that null changes at least one recorded margin "
                    "by more than that margin's own std. W0.DIAG measured "
                    "the mechanism: same-marginal correlated noise buys "
                    "gain_up 12.12 +- 1.20 where the stationary null reads "
                    "0.0095 +- 0.39, so every 'the null does as well as the "
                    "learner' reading was taken against a null strictly "
                    "weaker than an unlearned process can be.",
         falsified_by="Either branch, named in `claim_branch` (the BA.03 "
                      "one-bit-verdict lesson): (a) the correlated null does "
                      "NOT outscore the white null on the venue's own "
                      "outcome — W0.DIAG's result does not generalise past "
                      "food-seeking and Pile A dissolves; or (b) the "
                      "strongest null re-scores every recorded Pile A margin "
                      "within its own std — the under-nulling was real but "
                      "immaterial and the recorded findings stand as "
                      "written.",
         null_baseline="The incumbent stationary white null re-scored "
                       "against itself: every margin shift reads exactly "
                       "zero, so the metric's floor is the seed spread of "
                       "the fresh null runs, not an assumption.",
         metric="max_pile_a_margin_shift_over_own_std",
         budget=Budget.CPU, seeds=3, depends_on=["W0.DIAG"],
         control="The selection rule run BOTH ways and printed: if picking "
                 "the best null by the NULL'S own outcome ever selects a "
                 "DIFFERENT process than picking by the CLAIM arm's margin, "
                 "the old practice was fitting the null to the claim, and "
                 "that divergence is recorded on the row rather than "
                 "smoothed over. A selection that cannot be shown two-sided "
                 "cannot be audited.",
         kills="On FAIL branch (a): the null upgrade is retired, the Pile A "
               "findings keep their recorded nulls, and the repair for "
               "those venues moves to Pile B (the world), which reprices "
               "D10 fork (b). On PASS: every W-venue spec registered after "
               "this row states its null as the W1.00-selected process; "
               "nothing that already PASSed is re-opened — the upgrade can "
               "only RAISE a null, nothing that passed can pass more "
               "easily.",
         notes="REGISTERED 2026-09-06 (builder) from the Review FULL's "
               "published W1 design on the `w0-too-shallow` row — Pile A / "
               "measurement repair, runs on W0 AS BUILT, no world edit, no "
               "staleness bill. CROSS-CHECK (protocol step 1), constraints "
               "found and designed in rather than discovered by a dead run: "
               "(1) the colored null IMPORTS "
               "w0_diag_exploration_reaches_food's AR(1)-latent-through-erf "
               "construction verbatim — exact uniform(-1,1) marginals at "
               "every correlation time (the T3.06 matched-magnitude lesson; "
               "spectral synthesis cannot hold the marginal under "
               "clipping); (2) LC.03's venue scores are read from its "
               "RECORDED rows and artifacts "
               "(lc03_curves_seed{0,1,2}.json, on this box) — the standing "
               "do-not-relaunch order holds, this spec runs ZERO lives at "
               "the LC.03 envelope; fresh null runs happen only at the "
               "W0.DIAG envelope, whose BINDING known-answer control "
               "(wk5-N3) already validated the life_gain readout this spec "
               "inherits; (3) an instrument fault VOIDs rather than FAILs "
               "(T0.22); (4) no order-statistic bar frozen at a pilot n "
               "(T3.06 extreme-value lesson) — every bar is a t-statistic "
               "over seeds or a shift-over-std. FIRST of the W1 family by "
               "the Review's ordering: cheap, and it is the one that could "
               "show a third of `w0-too-shallow`'s evidence was "
               "under-nulled. W1.01/W1.03/W1.04 wait on "
               "`w1-world-edit-window` and are deliberately NOT registered "
               "with this pair.",),

    Spec("W1.02", 2, "Outcomes have resolution",
         hypothesis="W0's outcome metric, with censored lifespan REPLACED "
                    "by a graded outcome (integrated need-satisfaction from "
                    "drives.DriveLayer, or per-need time-to-first-failure), "
                    "resolves differences smaller than the effect any claim "
                    "built on it wants to make: >= K distinct outcome "
                    "values over N lives (K and N pre-registered in the "
                    "test before its first run), measurement quantum <= "
                    "MIN_GAIN/3 (MIN_GAIN imported from DP.04's test, 5.0, "
                    "unmoved), and censoring below a declared cap with the "
                    "censoring rate reported. This is DP.04's defect "
                    "promoted from a pilot's refusal to a world-fidelity "
                    "gate: 21 distinct lifespans over 3072 lives at a "
                    "quantum of 6.25 against a MIN_GAIN of 5.0 is a metric "
                    "that cannot see the thing it exists to see — the "
                    "derived >= 5791 lives/arm/task is the arithmetic "
                    "saying so.",
         falsified_by="Any conjunct: fewer than K distinct outcome values; "
                      "quantum above MIN_GAIN/3; censoring above the "
                      "declared cap; or — the one that matters — the "
                      "synthetic known-advantage control goes undetected, "
                      "in which case the replacement metric is exactly as "
                      "blind as the one it replaces and the venue still "
                      "cannot host a DP-family claim.",
         null_baseline="The incumbent censored-lifespan metric scored by "
                       "the same conjuncts on the same lives: it must FAIL "
                       "them (21 distinct values, quantum 6.25 vs "
                       "MIN_GAIN/3 = 1.67) — a resolution gate that cannot "
                       "distinguish the broken metric from its replacement "
                       "is measuring nothing.",
         metric="synthetic_min_gain_detected_and_quantum_ratio",
         budget=Budget.CPU_LONG, seeds=3, depends_on=["PS.01"],
         control="KNOWN-ANSWER, BINDING (wk5-N3): a synthetic arm with an "
                 "injected advantage of exactly MIN_GAIN must be detected "
                 "at >= 3 sigma by the replacement metric. CROSS-CHECK "
                 "CONSTRAINT, designed in: the injection is a DECLARED "
                 "arithmetic transform applied to the measured per-life "
                 "outcomes (the known-answer pattern W0.DIAG's control 1 "
                 "used), NEVER a direct write to the need vector — "
                 "PURPOSE_AND_SCAFFOLDING §5/G-A's accounting identity "
                 "makes any un-logged positive delta-e an ERROR by "
                 "construction, so a physics-level injection would void "
                 "the run it exists to validate. Second leg: the same "
                 "detector pointed at an injected advantage of ZERO must "
                 "NOT fire — a detector that fires on nothing measures "
                 "nothing.",
         kills="On FAIL: W0 keeps its censored-lifespan metric and the "
               "fact is recorded where it binds — DP.04 stays "
               "PILOT-BLOCKED, no DP-family claim is admissible in this "
               "venue, and the repair moves to the world-edit window "
               "(W1.03's sitting). On PASS: W1.01 and W1.03 quote this "
               "spec's measured quantum in their bars, which is why it "
               "must exist before either of them can be registered.",
         notes="REGISTERED 2026-09-06 (builder) from the Review FULL's "
               "published W1 design (`w0-too-shallow` row) — a Pile B "
               "defect with a Pile A-priced repair: runs on W0 AS BUILT, "
               "no world edit, no staleness bill. Censoring conventions "
               "IMPORTED from NEEDS_AND_DEATH.md / NE.08 rather than "
               "re-derived (the SO.08 Laplace-import precedent): every "
               "terminal transition carries a cause tag; censored and "
               "fatal lives are distinguished by the tag, never by the "
               "step counter; censoring rate reported; a mean over "
               "uncensored lives only is forbidden — it makes an agent "
               "that dies early look fast. K and N are pre-registered in "
               "the test docstring before the first run, with no "
               "order-statistic bar frozen at a pilot n (T3.06 "
               "extreme-value lesson). SECOND of the family by the "
               "Review's ordering: W1.00 first (cheap), W1.02 before "
               "W1.01/W1.03 because both quote its quantum.",),
    ...
    Spec("T0.28", 0, "The escalation tool can be shown catching a deadlock "
         ...
         hypothesis="`experiments/decisions.py` — the instrument that stands "
                    ...
                    "(12) that clause's checker reads a SELECTED population, "
                    "so identification is itself checked: a firing is found "
                    "by TWO independent channels — its own commit subject and "
                    "the `RESOLVED BY ARMED DEFAULT` record it cannot skip "
                    "writing — the real commit the subject channel missed "
                    "(`3b2e38b`, `D22`'s record, naming neither \"default\" "
                    "nor \"fired\") is found by the second, a commit both "
                    "channels see is not double-counted, a record-only commit "
                    "is AUDITED and not merely listed, omitting the second "
                    "channel does not narrow the first, and a declared firing "
                    "that no identified commit names is reported as missing "
                    "coverage; (13) and the same discipline on the other axis "
                    "— which BYTES rather than which commits: the working-tree "
                    "check asks git for the working tree (`diff HEAD`, for "
                    "`WORKTREE` in either case) and a named rev asks `show "
                    "<rev>` ONCE with no silent retry, asserted on the git "
                    "ARGUMENTS because a wrong invocation is invisible in a "
         ...
         falsified_by="Any property failing; the battery running fewer than "
                      ...
                      "at all. Also: a firing identified ONLY by its own "
                      "commit subject, so that a firing act whose subject "
                      "says nothing is never diff-audited while the report "
                      "calls the audit complete; a second channel that "
                      "widens the LIST without widening the CHECKING; a "
                      "commit both channels see counted twice; or adding the "
                      "second channel changing what the first one found. "
                      "Also: `WORKTREE` reaching `git show` instead of `git "
                      "diff HEAD`, so the check a firing iteration runs before "
                      "committing answers about the PREVIOUS commit — which is "
                      "what it did, printing `ok` over an uncommitted moved "
                      "bar — or a named rev being silently retried as a "
                      "working-tree diff.",
         null_baseline="THE ORGAN AS IT STOOD BEFORE 2026-08-30, kept "
                       ...
                       "And the organ as it stood at 11:0x on 2026-09-13 — "
                       "hours old, not weeks — when a firing was identified "
                       "by its own commit SUBJECT and nothing else: it must "
                       "miss `3b2e38b`, a firing act in this repository whose "
                       "subject declares nothing. And `_diff_of` as it stood "
                       "the same morning — `show(rev) or diff(rev)`, whose "
                       "second half `git show` made unreachable: it must ask "
                       "`git show WORKTREE` first. "
    ...
    Spec("T0.31", 0, "The backlog reader cannot be quieted by tidying the backlog",
         hypothesis="`experiments/review_queue.py` — the reader "
                    ...
                    "that really carries no promise; and (11) the OTHER HALF "
                    "OF THE TRANSACTION is read — arrivals, disposals and the "
                    "drain projection over a trailing window, from the file's "
                    "GIT history rather than its writable declared dates, so "
                    "that a desk which never breaks a promise because it "
                    "honestly re-arms every one of them is no longer "
                    "indistinguishable from a desk that is keeping up: a "
                    "disposal already banked before the window is not credited "
                    "twice, a DISPOSITIONED transition is counted separately "
                    "and NEVER as drain, UNBOUNDED is not by construction (a "
                    "desk that really disposes gets a finite drain), "
                    "back-dating a row's `routed` field cannot lower arrivals, "
                    "re-arming a DUE: changes no throughput number at all, "
                    "splitting one live row into two can only make the reading "
                    "worse, the whole reading moves no violation total, and an "
                    "absent baseline reports UNMEASURED rather than a clean "
                    "week.",
         falsified_by="Any property failing; the battery running fewer than "
                      ...
                      "to another reading as a repair; a row back-dated out of "
                      "the throughput window lowering the arrival count, a "
                      "design stamped DISPOSITIONED being credited as drain, a "
                      "split row improving the reading, a missing git baseline "
                      "reporting zero arrivals instead of UNMEASURED, or "
                      "`drain` reading UNBOUNDED for a desk whose disposals "
                      "genuinely outrun its arrivals.",
         ...
         control="See null_baseline. Named properties "
                 ...
                 "p12_a_disposition_is_not_an_execution and "
                 "p15_the_desk_is_measured_disposing_not_only_breaking must "
                 "be among the control's failures, or the row count is "
                 "somehow sufficient and this module guards a distinction "
         ...
         notes="SCAR (52nd audit, 2026-08-31, B4). The Review's Sunday FULL "
               ...
               "THE HALF-BLIND INSTRUMENT (69th audit, 2026-09-04, B1; P15). "
               "Every one of the ten classes above fires on a promise "
               "BREAKING, and `REVIEW_QUEUE.md` deliberately provides an "
               "honest way never to break one — re-arm the `DUE:` with a "
               "reason, as `decide_by` is re-armed. That hatch is right and it "
               "stays. Its unpriced consequence is that a desk which re-arms "
               "honestly forever is byte-indistinguishable, to every per-row "
               "class here, from a desk that is keeping up. On 2026-09-04 the "
               "file held 35 routed rows against 2 lifetime dispositions with "
               "30 arrivals in seven days, sixteen open rows were the SOLE "
               "declared repair for something an instrument printed red, and "
               "`run review-queue` printed `0 violations` — correctly. P15 is "
               "the aggregate the per-row classes cannot be: arrivals, "
               "disposals, `designed`, and `drain`, read from GIT because the "
               "declared `routed` date is writable and a rate that falls when "
               "you back-date a row teaches back-dating. It is a METRIC and "
               "never a violation (a slow week is legal; `piled_on`'s "
               "discipline one layer down) and it is RATCHETED (re-date, "
               "re-arm and split must each leave it unimproved), and its live "
               "integer `review_queue_net_arrivals` joins "
               "`ratchet_readings.json` so it cannot drift in silence. It "
               "departs from the audit's own wording in ONE place, on that "
               "audit's own evidence: `DISPOSITIONED` transitions are counted "
               "as `designed` and NEVER as drain, because a design is still "
               "live and still ageing and crediting it would make throughput "
               "reportable by writing prose — the two-meaning token as a "
               "rate.\n"
               "Deliberately declares NO `COVERS:` commitment. It guards the "
               "decision machinery, not a capability."),
    ...
    Spec("HR.1", 2, "The voice corpus is honest before anyone is scored",
         hypothesis="A speaker corpus exists on this box with >=8 enrolled and "
                    ...
                    "utterances, CROSS-MICROPHONE test material (enrolment on "
                    "one microphone, scoring on a second microphone recording "
                    "the same utterances simultaneously — this controls for "
                    "EQUIPMENT and NOT for occasion; amended 2026-09-23 from "
                    "'CROSS-SESSION', which no corpus on this box delivers — "
                    "110th audit FTB 1), and a "
         ...
         kills="HR.2, HR.3, HR.4. A speaker experiment on a leaky corpus "
               "measures the leak.",
         notes="VENUE AS DELIVERED (amendment 2026-09-23, 110th audit FTB 1 — "
               "recorded AFTER attempt 4 so this entry describes the "
               "experiment the ledger row actually holds; both venues kept on "
               "its face, the T4.06 STATISTIC_BOUND precedent): "
               "VCTK-Corpus-0.92, 110 speakers, TWO simultaneous microphones "
               "per utterance — mic1 (DPA 4035) carries enrolment and the "
               "planted leak, mic2 (Sennheiser MKH 800) carries the scored "
               "clean stratum — 20 enrolled / 40 unknown per seed, "
               "utterance-disjoint splits, UTT_CAP 100, 48 kHz resampled /3 "
               "by zero-phase polyphase inside load_clip, arm (a)'s per-clip "
               "quiet-floor whitening carried verbatim. Fetched under D19 "
               "with /data at 67 G free of 100 G (measured at fetch); zip "
               "verified byte-exact at the SERVED Content-Length "
               "11,747,302,977 (post-DSpace-migration; the 2026-08-09 probe "
               "recorded 11,749,118,645). The 'REJECTED: VCTK' line below is "
               "therefore HISTORY, written when /data free space swung "
               "725 MB-4.8 GB before D19 opened /data — kept for provenance, "
               "superseded by the measured figures above. Cross-mic controls "
               "for EQUIPMENT and NOT for OCCASION: both mics share one "
               "booth and one sitting, so occasion-borne cues (session "
               "ambience, reader state) remain uncontrolled — the caveat on "
               "the hr1-clean-stratum queue row. Bars, seeds and the probe "
               "are UNTOUCHED by this amendment: LEAK_EXCESS 0.05, "
               "CONTROL_FLOOR_EXCESS 0.15, 17 features, 3 seeds. "
               "ORIGINAL NOTES (2026-09-03, LibriSpeech venue — attempts 1-3 "
               "ran there; kept verbatim): "
               "Corpus: LibriSpeech dev-clean, CC-BY-4.0, 40 speakers "
    ...
    Spec("T0.35", 0, "An implementation dependency cannot go undeclared",
         ...
         notes="78th audit B1, the audit's highest-priority item. The "
               ...
               "header forbids. No coverage declaration: this is a harness "
               "spec, not a GOAL.md commitment claim — the first draft "
               "declared one against a commitment ('harness integrity') "
               "that does not exist and T0.21's p7 caught it within the "
               "hour, which is that spec doing its one job. (The marker "
               "word itself cannot appear in these notes: T0.21's parser "
    ...
    # SIZING: `lg_10._measure` is 0.01 s of process time per (seed, selector)
    # against a 124 KB cached verdict table — measured 2026-09-13 before this
    # spec was written. LG.13 runs 5 selectors x 3 seeds = 15 of them plus
    # run_spec overhead: ~2 s. Declared cpu<1min on that arithmetic, ~30x
    # headroom. NOTHING here buys an LLM verdict; see the notes.
    Spec("LG.13", 3, "The chooser earns the Language-routing seat, or the seat stays vacant",
         hypothesis="Among DECODE RULES reading the SAME frozen candidate "
                    "scores on LG.10's certified rig — plain softmax over the "
                    "whole pool (what this project shipped, never raced), "
                    "top-K softmax, top-K uniform, and meaning-mass (aggregate "
                    "the score per MEANING, then sample wording uniformly "
                    "within the chosen meaning) — at least two clear the "
                    "3-sigma learning gate over the score-blind null on mean "
                    "meaning-match across both frozen mouths, ONE wins "
                    "outright by the 1.5-sigma margin, AND that winner is "
                    "ELIGIBLE to hold the seat: wording variety at or above "
                    "LG.10's VARIETY_MIN on every seed, the state-free null "
                    "prompt still not tracking state under it "
                    "(NULL_MATCH_MAX, both models), and no fabricated line "
                    "ever drawn.",
         falsified_by="The bakeoff reaches no decision, or reaches one whose "
                      "winner is INELIGIBLE — a rule that buys meaning-match "
                      "by killing the sampler's freedom (variety below the "
                      "floor, so meaning-invariance across draws is vacuous), "
                      "by sharpening the state-free null until it tracks "
                      "state too, or by drawing a fabrication. A TIE also "
                      "falsifies: 'the choice does not matter yet' is not a "
                      "title, and this seat was already filled once by "
                      "accident. Under every branch the seat stays VACANT and "
                      "the finding routes.",
         null_baseline="SCORE-BLIND UNIFORM selection over the identical "
                       "gate-passed pool: the chooser removed entirely, which "
                       "is the quantity this race is about. Chance is ~3/17 = "
                       "0.18 per draw. Every arm's sigma is measured against "
                       "it.",
         metric="match_both", budget=Budget.CPU_FAST, seeds=3,
         depends_on=["LG.00", "ME.9"],
         control="THE STATE-FREE PROMPT — LG.10's own declared null "
                 "(NULL_ASK: no core-selected intent in the prompt), read "
                 "under the INCUMBENT selector and scored on this spec's "
                 "ruler. Entered through `run_bakeoff(controls=)`, so it is "
                 "scored and never competes; if it CLEARS the 3-sigma gate "
                 "the verdict inverts to VOID, because the scaffold would "
                 "then be leaking the intent and no arm's meaning-match would "
                 "be evidence about its chooser.",
         kills="The unexamined plain-softmax default's claim to the seat. If "
               "it loses, the seat goes to what beat it; if nothing wins "
               "outright and eligibly, the seat is not filled by the "
               "incumbent's tenure.",
         gate_mode="screen",
         screen_rationale="The arms are OBSERVABLES, not learners, and the "
                          "rig makes that structural rather than asserted: "
                          "every arm is a deterministic function of ONE "
                          "already-cached verdict table (1588 frozen "
                          "log-probabilities, content-hash keyed) that no arm "
                          "can perturb, and no arm has a parameter fitted to "
                          "anything. A low score is therefore a property of "
                          "the RULE and not evidence that its run broke, "
                          "which is exactly the case `validity` mode would "
                          "mis-VOID. The gate is unmoved at 3 sigma and "
                          "MIN_FINISHERS still applies. Declared in advance "
                          "and not expected to bind: LG.10's published "
                          "incumbent reads match 0.60-0.78 against a 0.18 "
                          "null, so the mode is on the record before any arm "
                          "number exists rather than switched on after a "
                          "VOID.",
         notes="THE SEAT: `docs/CHAMPIONS.md` opened *Language routing (what "
               "he says, and which task a command becomes)* on 2026-09-13 and "
               "recorded it VACANT with three red arena members (LG.10, "
               "LG.12, T2.15) arbitrated by nothing. SYSTEM.md's standing "
               "rule is that no architectural seat may be held without a "
               "registered existing challenger; law 3 says a fork whose arms "
               "can all be run is an experiment nobody has written yet. "
               "THE SEAM IT RUNS ON: `lg_10._measure(select_fn=)`, committed "
               "2026-09-13 defaulting to the shipped `_draw`, so LG.10's "
               "certified path is byte-identical. ARMS MAY SWAP ONLY THE "
               "SELECTOR — the world, the pool, the verification gate, the "
               "prompts, the cached scores and every metric are LG.10's, "
               "imported, never re-derived. "
               "THE TEMPERATURE KNOB IS DELIBERATELY NOT AN ARM, and this is "
               "the one design choice a reader should check. "
               "`lg10-mouth-fidelity-vs-freedom` (Review DAILY 2026-09-08) "
               "ruled *do not fit T — both endpoints are already paid for*: "
               "T=0.25 gives match 0.98 and VOIDs on variety, T=1.0 gives "
               "variety 1.0 and FAILs on match. Racing T would re-buy a "
               "measured answer and would be knob-fitting on a failing "
               "spec. Every arm here holds TEMP at LG.10's registered 1.0 "
               "and differs only in DECODE STRUCTURE, which is the direction "
               "that same disposition named as legitimate (*'the mouth needs "
               "a stronger chooser — bigger frozen model, structured "
               "decode'*); the bigger model is the half that costs verdicts "
               "and is not taken. Greedy argmax is excluded for the opposite "
               "reason and it is named so the omission is not silent: it is "
               "INELIGIBLE BY CONSTRUCTION (one utterance five times => "
               "variety 0) and carries the fewest constants, so entering it "
               "would manufacture SO.10's finding rather than measure "
               "anything. "
               "WHAT THIS SPEC MAY NOT DO TO LG.10: nothing. LG.10's bars, "
               "its FAIL and its standing measurement are untouched and "
               "survive any verdict here. A PASS says a chooser holds the "
               "SEAT; it does not say the shipped mouth pipeline preserves "
               "meaning, and converting one into the other would be the "
               "redesign-a-failing-spec move that disposition forbade. If an "
               "arm wins, whether LG.10 is owed a successor under it is the "
               "Review's call and routes as a queue row. "
               "A TIE IS A FAIL HERE, which is STRICTLY HARDER than SO.10's "
               "cost-resolved tie and is pre-registered before any number. "
               "`run_bakeoff` resolves a tie by declared cost and, at equal "
               "cost, `min` returns whichever arm sorted first — an "
               "arbitrary pick. Costs are declared because the primitive "
               "requires them and the resolution is RECORDED, but the seat is "
               "not awarded on it. This does NOT pre-empt "
               "`so10-tie-break-hands-the-seat-to-an-ineligible-arm` (DUE "
               "2026-09-17), which asks whether `bakeoff.py` itself must know "
               "about eligibility: nothing in the primitive is touched here "
               "and this clause binds one spec. "
               "ELIGIBILITY IS NOT THE SCORE — SYSTEM.md's "
               "SCORED-AND-INELIGIBLE rule: every arm's number is recorded "
               "whatever its eligibility, and the seat is a second gate on "
               "top. "
               "NOT ONE BAR IS THIS FILE'S OWN. The gate (3 sigma) and margin "
               "(1.5) are `run_bakeoff`'s defaults; VARIETY_MIN, "
               "NULL_MATCH_MAX, LIVENESS_MIN, TEMP and S_DRAWS are LG.10 "
               "constants imported unmoved. The one constant introduced is "
               "K = N_PHRASINGS = 3, DERIVED from the pool's own granularity "
               "(three wordings per meaning) rather than chosen, so top-K "
               "admits exactly one meaning's worth of candidates. "
               "NO SPEND: run() loads no model and buys no verdict. The "
               "artifact's keys are content-hashed over (model, revision, "
               "scaffold, prompt, candidate), so a missing verdict is a VOID "
               "and never a silent re-purchase — `lg10-mouth-fidelity-vs-"
               "freedom`'s no-spend clause is enforced by the rig, not by "
               "this sentence. "
               "WHAT A PASS BUYS: a champion seated in `docs/CHAMPIONS.md` BY "
               "VERDICT with this row as the deciding run. It claims no new "
               "capability for Jack — the kind is `rule` on purpose, so "
               "nothing here can move a commitment's `n_pass`. "
