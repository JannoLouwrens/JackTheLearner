...
THE LIFE, in two declared phases:
  FORAGE  drive between obj0/obj1 (and the apple, only if it has left its
          ...
          drowning cannot end the life the claim needs alive.
...
refused; the forager starving before the hour (the fixture failed to buy the
exposure — the world working as calibrated refutes the SCRIPT, not the
harness); the fast phase failing to kill by its cap (ditto, inverted). Each
...
def _experiment(seed: int) -> dict:
    ...
    if j0 is None:
        return m
    m.update(_feasibility(seed, j0, alpha))
...
def _check(m: dict, c: dict):
    ...
    if m.get("survived_hour") != 1.0:
        # The forager starved before the hour: the FIXTURE failed to buy the
        # 240x exposure, so the harness claim was never tested.
        return Status.VOID
