...
LADDER: list[Spec] = [
    ...
    Spec("T3.06", 3, "Ablate curiosity", hypothesis="Removing intrinsic reward reduces unprompted coverage.",
         ...
         control="Extrinsic reward PLUS a time-permuted, magnitude-matched "
                 ...
                 "its information is destroyed — placed in the CLAIM'S SEAT "
                 "against the claim's hardest comparator: it must NOT clear "
                 "the C-RANDREW bar the curious arm must clear "
                 "(ctl_randrew = cov(shuftask) - cov(random) < DELTA_MIN). If "
                 "it does, clearing that bar measures 'any reward shaping "
                 "beats a random walk' rather than curiosity, the contrast "
                 "cannot attribute, and the run is VOID whatever the "
                 "experiment arm did. Declared 2026-08-29 with the "
                 "implementation; adding a control is strengthening. "
                 "RESEATED 2026-09-22 under Review 1^10 item 2(a): the v2 "
                 "form gated cov(shuftask) - cov(task) < DELTA_MIN, which was "
                 "RED on every seed (0.1072 vs 0.05) and so was never a "
                 "passing control; that question is now asked INSIDE the "
                 "claim as C-NOISE (cov(curious) - cov(shuftask) >= "
                 "DELTA_MIN), a strictly harder bar, and the must-fail arm "
                 "moved to the conjunct that now carries the load. The count "
                 "of must-fail arms does not fall.",
