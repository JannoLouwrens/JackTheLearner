...
| Control architecture (D1) | **VACANT** — prior holder's evidence voided (T0.14) | — | **D1.0 (REGISTERED 2026-09-01 by D1's firing commit — the arena existed as a phantom for 22 days and now resolves in BY_ID)** + T2.21 (still unregistered by the 08-13 decision) | **Challenger list corrected per D1's default (fired 2026-09-01): frozen-trunk+head (option A) is STRUCK as unconstitutional — the PLASTIC-ONLY decree postdates nothing and option A postdates the decree. The permitted challengers are D1.0's four arms: A-prime (learned control head on plastic-trunk features), B (split value/policy trunks), C (end-to-end at more steps, UNTESTED not refuted), D (transformer out of the control path — a D win forecloses DP.02, cost recorded with the verdict).** |
