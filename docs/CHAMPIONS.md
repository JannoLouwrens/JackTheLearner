...
| Sensory fusion | undecided — token-trunk favourite | — | UB.10 (**PARKED 2026-08-20** — the recipe probe's both-fail branch fired, no uniform recipe trains all six matched-param arms, arm redesign routed to the Review; the one-diagnostic cap is spent). Its prerequisite **`UB.9` is now `PASS but STALE`** (passed 2026-08-12 at fused 0.993 vs nulls at chance; `impl_sha` moved `fe3cc27…` → `5ac2b07…`), and stale means the seat's only evidence is about older code. `run blocked` ranks UB.9 **#3, frees 5, blocks 7** — Review 2026-08-21 | six arms, matched params |
