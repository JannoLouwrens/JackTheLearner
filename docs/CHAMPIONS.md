...
| seat | champion | held | arena | challenger status |
|---|---|---|---|---|
| Language routing (what he says, and which task a command becomes) | **VACANT** — no component claims it, and two designs are already contesting it in the open | — | `LG.10` (*Jack chooses what to say; the LLM only chooses how*, **FAIL**), `LG.12` (*he speaks correctly or he is silent — the abstaining mouth*, **FAIL**), `T2.15` (*free-form language routes to the right task*, **FAIL**) | **Pre-routed to this audit by the Review of 09-10 and owed since.** `LG.12` was registered on 2026-09-13 as *"LG.10's sibling and provably NOT its repair"* — that sentence is the definition of a challenger, and the project wrote it down while having nowhere to put it. Two competing mouth designs, both FAIL, arbitrated by nothing. GOAL.md requires the mechanism twice over and in opposite directions: *"language is how he reports it, never where it comes from"* (the mouth) and *"one sentence can spare him a thousand falls"* (the ear that must become an act). Distinct from `Language grounding` (word → lived skill, which is what a word MEANS) and from `Language model` (the parent LLM, which is decided BY DECREE): this seat is who chooses the CONTENT and the DESTINATION. All three arena members are red, so the ring is open and nobody is defending it |
...
- SEAT: Language routing (what he says, and which task a command becomes) | HELD: VACANT | ARENA: LG.10, LG.12, T2.15
