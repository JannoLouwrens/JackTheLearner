...
|---|---|---|---|---|
| Learning core | PPO (tuned per Moon et al. — F9) | **DEFAULT, never defended** | LC.00–LC.06 (LC.00–LC.02 + PS.01 PASS; **LC.03 landed VOID 2026-08-14 07:36 `8ec4be8` — "run did not test the claim; not a refutation" — and is now STALE by `IMPL_DEPS`; it must be re-run**. Status corrected by the Review 2026-08-20; this cell read "IN FLIGHT" for six days after the run ended) | DreamerV3-class + others — screening only: LC.03 declares no winner; the seat's actual match is LC.04, still ahead. **SEAT FLAGGED STALE (rule 4, Review 2026-08-20): the default champion has never been contested, and its screening round has now been VOID for six days with nothing scheduled.** |
...
| Smell (olfaction) | **VACANT — sense fixture certified, seat unclaimed**: `SM.01` (field fidelity) **PASS 2026-08-11** | — | `SM.02` (occluded food, the claim) **implemented 2026-08-20 `eb42dd3`; pilot 1 returned NON-LEARNING (all six DQN arms 83–100% timeout on all three seeds, 1.56 Kaggle-h); a shaping repair is in progress and UNCOMMITTED. Gates still provisional — `run()` hard-refuses until `_GATES_FROZEN=True`** | finds food/fire/decay through occlusion |
