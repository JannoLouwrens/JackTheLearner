...
|---|---|---|---|---|
| Learning core | PPO (tuned per Moon et al. — F9) | **DEFAULT, never defended — and now measured a NON-LEARNER in W0** | LC.00–LC.06 (LC.00–LC.02 + PS.01 PASS; **LC.03 v2 CONCLUDED 2026-08-23 21:11 — VOID `fewer than two learners (1 cleared)`, the pre-registered fork (ii) fired: `wm-latent` was the SOLE 3σ learner (t_null 4.65, t_twin 4.00, needs_rise +0.022, clt +92.2 s), both PPO arms and dreamer-xs failed to clear the null at the 4× envelope with a clean rig. No v3 — the fork priced the data-starved flags. Cell updated by the builder at the harvest, per queue protocol step 6**) | The screening design was the finding: W0 does not discriminate these five cores at a reachable envelope. LC.04 — the seat's actual match — is blocked behind an LC.03 PASS the fork says will not be manufactured. **The seat is PENDING D10 (DECISIONS_NEEDED, 2026-08-24: accept-one-learner / redesign W0 / redesign arms — the owner's call). `wm-latent` is NOT seated: that is D10 option (a), not the harvester's to take.** |
