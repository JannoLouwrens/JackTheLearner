...
## An organ that publishes to a page nobody reads has not escalated anything — and "current state" is a delete key on a timer (2026-09-04, builder, the 69th audit's B2)

Two documents in this repo hold questions for the owner. One of them —
`docs/DECISIONS_NEEDED.md` — is append-only, has a parser, arms every entry with
a `class`, a `default` and a `decide_by`, and is checked by a tool the overseer
runs at the top of every audit. The other — the `## FOR THE OWNER` section of
`docs/PROGRESS.md` — is where the project's chief-scientist organ actually
writes, and it is **current-state by design**: the next Review rewrites the page
whole.

On 2026-09-03 the Review published its largest strategic recommendation there
(*"W1 stops being a queue row and becomes the project's stated stage"*), against
four claim-dead constitutional commitments and six independent instruments. At
24 hours old, its own successor's rewrite deleted it. `decisions --check`
printed `ratchet ok (0/10 undeclared)` the whole time — **true of the file it
read, false of the system.** Only an overseer noticing by eye kept it alive, as
`D21`. The same page lost a second ask the same way in the same window.

**The generalisable shape: an escalation is only as durable as the document it
lands in, and no organ audits the durability of its own outbox.** The Review had
no reason to think it had failed — it wrote the words, in the section named for
exactly that purpose, and the words were gone. Compare the neighbouring scars:
`D19`'s NO-FETCH hold lived in prose and `coverage` went on advertising `HR.1`
as fillable; `HR.5`→`HR.6` was a blocker written as a sentence and every ranker
was blind to it. Same disease, one layer up — **a blocker written as prose is
invisible to instruments; an ask written on a page that rotates is invisible to
everyone, including its author.**

Three things generalise beyond this instance:

**1. Every organ's outbox needs a reader, and it must be a different organ's
reader.** `PROGRESS.md` had none: `decisions.py` read only its own file, and
`overseer_prompt.md`'s READ FIRST did not name the page — a fact the overseer
itself had measured and written into `D15` on 08-29, then not acted on for six
days, because the organ that would fix it may not edit its own script. Two
organs each correctly doing their own job is not coverage of the seam between
them.

**2. When you cannot decide which items are asks, do not build the classifier —
invert the burden.** The audit ordering this check hand-classified the same four
paragraphs two different ways one paragraph apart: a "here is the Sunday order I
will take" item counted as an ask on 09-03 and as mere scheduling on 09-04. If
the rule's own author cannot apply it consistently to four paragraphs, a regex
will not either. So: **every item is an ask until it DECLARES otherwise**
(`NO-DECISION: <reason>`, reason required). Silence is reported; exemption is
written down. That is yesterday's partition lesson applied to a document instead
of a dataclass — the silent side is the dangerous one, so make silence the loud
side.

**3. Match by QUOTATION, not by similarity.** The check asks whether a 6-token
verbatim span of the ask appears in a decision file. That was not chosen for
convenience: quoting the recommendation into the entry, blockquote and commit
citation, is what the overseer actually DID when it lifted this scar onto the
owner's desk. A check whose green is *the repair the system already performs*
needs no similarity threshold, cannot be gamed by paraphrase, and makes the
entry that silences it a faithful record of what was asked.

**And the trap that nearly ate the whole guard, found only by pointing it at the
live page:** `\bD(\d{1,3})\b` matches `D1` inside `D1.0`. `D1` is a decision;
`D1.0` is a spec. The one true positive on the board — the 09-03 `run blocked`
ask — read as ROUTED TO `D1` purely because its prose mentioned `D1.0` twice,
and the brand-new check reported a clean board on its first live run. `_DECIDE`
already carried this scar with different numbers (`"nest"` inside `"ho-nest"`
credited a shelter commitment). **Rule: a new id-matching regex must be pointed
at the live corpus and its NEGATIVES inspected before you believe a zero — a
guard's first green is the least trustworthy output it will ever produce**, and
the near-miss ids in this repo (`D1`/`D1.0`, `T2.0`/`T2.01`, `SM.0`/`SM.02`) are
generated by its own naming scheme, so this collision class is permanent.

## A KILL bound and an ADMISSION estimate are two quantities that share a unit —
## single-sourcing them looks like the anti-duplication virtue and is a category
## error (builder, 2026-09-04, building the 69th audit's B4)

`gate_cpu_child` refused a run when `spec_child_timeout_seconds(spec)` exceeded
the day's remaining seconds. That function is the arithmetic `run.py` uses to
KILL a child, and reusing it was deliberate — it is the same discipline that
put `BUDGET_SECONDS` in one module after T0.14's pasted-constant scar, and the
comment beside it says so.

It was wrong, and by a lot. A kill bound must be the WORST case, because
killing early destroys finished science (T2.01's 66.7-minute kernel died at a
flat 60-minute cap). An admission estimate must be the EXPECTED case, because
refusing early destroys throughput. Point them at one constant and the worst
case wins both jobs: measured across the 108 runner-lane cpu specs with a
recorded duration, the median ratio of allowance to actual cost is **257x**
(`W0.DIAG` 0.02 s against 10800 s). The consequence was live and nobody had
connected it to its cause — **3600 s of routine housekeeping, 6.25% of the
ceiling, foreclosed 53 of 152 CPU specs.**

**The rule: single-sourcing is right for one quantity used twice, and wrong for
two quantities that happen to have the same units.** The test is not "are these
numbers equal today" — they were — it is "does a reason to move one give a
reason to move the other?" Here, tightening the kill bound risks losing
science; tightening the admission estimate risks nothing but a marked overrun.
Opposite failure modes on one constant is the tell.

**The safe shape for replacing an enforced bound with an estimate**, and it is
worth copying because it makes the change unarguable: return
`min(bound, estimate)`, keep the enforcement untouched, and assert the clamp as
a PROPERTY over the whole population (`T0.33` P12: `est <= enum` for every
registered runner-lane cpu spec). Then the change is provably never stricter
than what it replaced, the true worst case is exactly what it always was, and
an undershoot costs a marked overrun rather than an unbounded run. P13 supplies
the other half — on a day drained to exactly halfway between estimate and
bound, the OLD path refuses where the new one admits — because without it P12
is satisfied by an estimator that changed nothing.

**And the finding that outranks the fix: an instrument improvement validated on
the measured population can be worth ZERO for the case it was built for.** The
count fell 53 -> 36, and `n_foreclosed_unmeasured` read **36** — every residual
foreclosure is a spec that has never run, so there was nothing to project from.
The audit armed B4 "for the first morning after the 09-06 Review orders CPU
work", and the specs a resolved redesign orders are *new* ones, which is
exactly the population the fix cannot reach. The residual has a different
cause and a different owner: `CPU_DAY_CEILING_S` (57600 s) is **1.067x** the
largest legal child (54000 s), a pure inequality with no measurement in it, and
every repair for it raises or splits a tenant protection — law 4, the owner's.

**Generalised: after fixing an estimator, count how many of the surviving cases
your fix COULD have reached.** If that number is zero, you have improved the
instrument on the population that already had evidence and left the population
the instrument exists for untouched — and the metric moving (53 -> 36) will
