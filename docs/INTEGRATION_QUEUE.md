...
|---|---|---|
| **FIELD_WATCH wk4-N3 → `W0.DIAG`** (the W0 shallowness diagnostic) | **W0.DIAG** — registered, run, **PASS** | **DONE 2026-08-31; marked 2026-09-01** (the executing iterations never performed step 5 — a completed top-of-queue row read "to register" for a day, the same status-token class the DISPOSITIONED/ACTED split closed in `bf64a85`). REGISTERED `853ab19` (cost class cpu<10min, previously EMPTY, now filled). Attempt 1 **VOID** on the un-piloted repeat null — lesson filed in the commit: *a pilot certifies floors per condition, not per spec*. Attempt 2 **PASS** `2a31a8e` (N=4800): the β-scheduled correlated null buys life in W0 **through food** — margin_up **+12.1 s** (t≈18.4), eats_up **1.0** vs eats_random **0.33**, plain/repeat nulls flat, mirror −11.1 s; **the wk5-N3 known-answer control was carried as BINDING and cleared (t≈11)** before the W0 reading was believed, exactly as ordered. Attempt 3 clean-tree re-buy PASS `dad5f27` (2026-08-31T11:19:57). WHAT IT DECIDED, per the pre-registration below: the branch fired — part of what the shallowness instruments recorded as "the world does not reward X" is "white-noise exploration never reaches the food"; the finding lands in D10 fork (b) and the `w0-too-shallow` row (where the Review 08-31 counts it), and is already carried into the LT registration row below ("white noise dithers, correlated action reaches"). Original order kept for provenance: **TOP OF QUEUE, put here by the Review 2026-08-31 (DAILY), and the reason is a finding about this file.** The Review ACCEPTED and ORDERED this diagnostic on 2026-08-25 and sequenced it *"BEFORE any W1 world redesign"* — then wrote the order as **prose in the wk4 consumption section below**, with no spec id, no cost class, and no row in this table. The builder reads this table top-down. Six days, zero iterations touched it, and on 08-30 the `w0-too-shallow` design came due with its own stated input never ordered into existence. **An accepted nomination that is not a queue row is a nomination nobody accepted.** THE UNIT: register `W0.DIAG` (cost class **cpu<10min**, which `coverage` reports as EMPTY and fillable by one spec), then implement and run it. THE DESIGN, unchanged from wk4-N3 as accepted: run `LC.03`'s existing `random` and `random-repeat` nulls against a **β-scheduled colored-noise random policy** in W0 and read `life_gain`. It is NOT a new world and NOT an exploration arm on `A0`/`A1` — that half was REJECTED on 08-25 for the PPO likelihood-ratio reason and stays rejected. **CARRY wk5-N3 AS A BINDING CONTROL** (see the wk5 section below): the diagnostic must first reproduce a KNOWN answer on a world whose relative shallowness we already know, and fail loudly if it does not, before its W0 reading is believed. That control costs the same CPU-minutes the diagnostic already costs, and without it this is an unvalidated instrument being used to attack eight validated ones. **WHAT IT DECIDES:** if a β-scheduled null beats plain `random` on `life_gain`, part of what nine instruments recorded as "the world does not reward capability X" is actually "the exploration process never reaches the food" — and that lands in D10 fork (b) and in the `w0-too-shallow` row directly. If it does not, the shallowness finding survives its cheapest attack, which is what a control is for. |
...
| FROZEN_VS_PLASTIC.md §7.3 (the PL family) | PL.00, PL.02 | **PARTIALLY REGISTERED 2026-08-30** (`4f8d99a`, registry 198→200), OVERSIGHT B5 — seventh audit asking, 22 days. CROSS-CHECK (step 1) over docs/research/*.md + docs/LESSONS.md + docs/DECISIONS*.md for `frozen|plastic|reshaping gain|throughput floor`: **ONE REFUTATION FOUND and acted on** — the PLASTIC-ONLY decree of 2026-08-09 collapsed §7.3's four-arm frozen-vs-plastic contest to a single admissible arm, so PL.00 and PL.02 are registered **corrected per the refuting analysis**, exactly as `CHAMPIONS.md`'s own "WHAT STILL RUNS" paragraph had already written the correction (PL.00 becomes a feasibility check on the pure encoder; PL.02 measures what the plastic path BUYS). **NO THRESHOLD EDITED**: the 5.0 sim-s/real-s floor is LC.02's and LEARNING_CORE §5.0b's, PL.02's bootstrap-CI-excluding-zero is §7.3's. Second cross-check result, recorded because it fires the next day: D1's option (ii) would narrow the decree to *sensory* towers (DECISIONS_NEEDED.md:791) — both specs are about sensory encoders, so neither is affected by either fork. VERIFY (step 2): 2 new ids, no collision, no prefix shadowing, all depends_on resolve (T0.07, PG.6, PG.1 all PASS). **PL.01/PL.03/PL.04/PL.05 DELIBERATELY NOT REGISTERED** — they are the arms and the arbitration of the contest the decree ENDED, and PL.01's own notes require amending LEARNING_CORE.md's U2 first (its arithmetic excludes every frozen tower by wording rather than evidence). Registering an arbitration whose arms are unconstitutional would be specifying work nobody may do; the ids stay free and §7.3 keeps their text. **Step 4 DONE: PL.00 implemented and run the same day — FAIL** (3 seeds, 223 s, every rig gate green, identity control clean at 0.72% shift). The from-scratch encoder reached 4.145 sim-s/real-s against the 5.0 floor, but **render-only with no encoder reaches 4.231** and a no-op 4.246: an eye frame costs 40.0 ms and the seat holder costs 1.045 ms, **2.6% of its own render**. The decree's pre-registered RE-OPEN TRIGGER fired and is escalated as written (**D17**) with the decomposition attached. Seats UNFALSIFIABLE went **7/7 → 5/7**. Step 6 n/a (no bakeoff completed). |
| CURIOSITY_BAKEOFF.md | LT.01–LT.09 | **REGISTERED 2026-08-31** (registry 202→211, verbatim from §3.3, no threshold edited — 54th-audit B1). CROSS-CHECK (step 1) over docs/research/*.md + LESSONS.md + DECISIONS*.md for `curiosity\|ladder\|ICM\|RND\|learning progress\|metra\|disagree`: **no refutation**; three findings carried onto the record instead. (a) LESSONS' "smuggling" entry (the CU.3 shape: ICM/RND cast as controls-that-must-fail so learning-progress can never lose) is ANSWERED by this family, not repeated — LT.04 races three genuine candidates (`disagree`, `lp`, `metra`) and the seat holder `lp` can lose its seat there; ICM/RND enter only to certify the trap detectors, with failure modes measured in the literature (1808.04355) and in PG.4. (b) wk3-N2's CIG condition BINDS the implementer: CIG does not fit LT.04's model-free arm (O(T³) at T=2000) — do not let it leak in. (c) LT.04's "fewer than two arms clear LT.03 → VOID 'fewer than two learners'" is the exact fork LC.03 fired on 08-24; the precedent (no re-roll, route to Review) travels with it. Also noted: W0.DIAG's finding (white noise dithers, correlated action reaches) is about W0's exploration process and does not touch the playground null floors, which LT.01 re-measures itself. VERIFY (step 2): 9 new ids, none in BY_ID, no prefix shadowing (two-digit ids by design — the lt_1_/lt_10_ glob collision is pre-answered in the doc), all depends_on resolve (PG.1/PG.3/PG.4 PASS; T2.01/T2.02 gate LT.08 as designed). `champions --check` moved UNFALSIFIABLE **5→4** and phantom-arena **2→1** in this commit; Curiosity signal now reads "real arena that has never run (declared pending, not a violation)". **Step 4 NOT done this iteration: LT.01 is the next unit** — CPU_LONG (~20 CPU-min per its own kills line), deps all PASS, and every threshold in the programme is set from its measured floors. LT.08 stays honestly blocked behind T2.01/T2.02. |
| D1_CONTROL_ARCHITECTURE.md | D1.0, T2.21 | **PROCESSED 2026-09-03 (builder) — steps 1–2 executed, step 3 correctly REFUSED; nothing registered.** CROSS-CHECK (step 1) found both ids dispositioned OUTSIDE this file while the row still read PENDING. **D1.0 is already REGISTERED** (2026-09-01, by D1's armed-default firing commit per 03f31cf's executor line) — in a DIFFERENT, constitutional form than this doc drafted: the four permitted arms A′/B/C/D, with option A (frozen-trunk+head, this doc's A4/A5 shape) STRUCK as unconstitutional under PLASTIC-ONLY (DECISIONS_RESOLVED.md §D1) — and it has RUN (attempt 1 VOID 2026-09-01, 16.17 GPU-h; gate redesign owed by the Review 09-06 on the `d10-*` rows; re-dispatch forbidden until an adopted gate). **T2.21 stays DELIBERATELY UNREGISTERED**: decided against 2026-08-13 (`a3b12f6` choice (b)) and reaffirmed by D1's resolution (*"T2.21 remains unregistered by that same decision"* — DECISIONS_RESOLVED.md:220); registering it as written would (a) put a second spec on the claim D1.0 now owns (the W.6/NE.08 two-specs-one-claim class), (b) enter two arms whose 57M-frozen-resident shape the resolution STRUCK, and (c) cite a prerequisite "D1.0" that no longer means this doc's R1–R6 repairs spec. The PL.01/PL.03–05 precedent governs: the id stays free, the doc keeps its text. **What SURVIVES the doc as live design input** — noted onto `d10-learning-gate-uses-two-different-denominators` (2026-09-03) for the Review's gate redesign: §3/§6's external SB3 reference arm with a harness-indicting ≥450 VOID gate, per-arm untrained twins as the binding null (already half-adopted in D1.0), and the shuffled-z control for any percept-fed arm. Step 4 n/a (nothing of this row is runnable today). RULE, so the disease is named where it lived: a commit that dispositions a queue row's ids elsewhere (a firing default, an audit striking a citation, a resolved decision) must update THIS TABLE in the same motion — this row read PENDING for 21 days after `a3b12f6` and 2 days after D1.0 registered AND ran, and today's iteration was one cross-check away from registering a spec against a standing decision. |
| HEARING_BAKEOFF.md | HR.1–HR.8 | **REGISTERED 2026-09-03** (`1733280`, registry 217→225), verbatim from the doc's §2.4/§3.1/§3.2/§4.1/§4.2/§5.3 drafts; notes-only additions (COVERS annotations, the NE.00 precedent, + one constitutional annotation on HR.6's A6: the frozen CED-tiny tower runs SCORED-AND-INELIGIBLE per SYSTEM.md's 08-24 amendment, so §1.3's null hypothesis is decided by bakeoff without contradicting PLASTIC-ONLY). CROSS-CHECK (step 1) over FROZEN_VS_PLASTIC/UNIFIED_BRAIN_BAKEOFF/LESSONS + DECISIONS*: **no refutation** — jobs (a)/(b) (Whisper, speaker-ID) live OUTSIDE the brain (LLM-parent status), and the PG.7 normaliser lesson is carried into HR.5's shape-only band features. VERIFY (step 2): 8 new ids, no collision, no prefix shadowing, all depends_on resolve (PG.2/PG.5/PG.7/ME.9 all PASS). `cpu<10min` went EMPTY-no-path-in → **fillable (HR.1, HR.5, HR.7)** — the inventory repair the W34 post-mortem named; UNREACHABLE_BASELINE 85→90 raised in the same commit with the justification in its growth log (HR.2/3/4/6/8 deliberately blocked behind same-family parents, the GEN.02-09 shape). **Step 4 DONE: HR.5 implemented and run the same hour — FAIL, attempt 1, exactly as pre-stated in its docstring** (T2.05 discipline): classes_present 1.0/4 on every seed (impacts only), water entry SILENT (8 underwater basin thuds, 0 water-kind events — Water is a force field, the floating apple never even reaches the basin on entry), creak/roll are placement onsets (sustained_roll 0; the 8 "sustained" creak-scenario events are the apple rolling off the plank and thudding on the FLOOR), no `kind` label, no `is_self` flag — while the instrument proved itself alive (two-pitch probe 1.0, f0 ratio 2.75, every seed) and the position-only control SUCCEEDED at 0.708: sound-kind IS geography today, as the registry control predicted. The hearing commitment's world half is now measured, and the row names the exact repair machinery (sustained noise voice from persisting contacts, surface-crossing water burst, kind + is_self labels — see HR.5's docstring forward contract). §8.1's two lessons filed in LESSONS.md and §8.2's disk escalation filed as **D19** (speech arm NO-FETCH default, decide_by 09-14) — all three had been staged-not-landed since 08-09. Step 6 n/a (no bakeoff completed). NEXT in this family per §6.3: HR.7 (cpu<10min, runnable, no disk), then HR.6's CPU arms; HR.1 waits on D19. |
| LANGUAGE_GROUNDING.md | LG.* | **PARTIALLY REGISTERED 2026-08-25** (`ed2d969`, registry 183→187, OVERSIGHT B1(a)): the completeness check CONFIRMED the truncation — §2.2–§11 are headers with no body, §7 ("registry entries") is empty — so what was registered is exactly the owner-designed material that EXISTS: **LG.00** (anti-puppet, the GOAL.md-cited asymmetry, verbatim from this file's GAP-FILL), **LG.10** (who chooses the words, verbatim), **LG.02** (THE LIAR TEST as specified on this row: track-record divergence, mid-life swap control, attribution-stripped null), and **LG.01** (the certification fixture LG.00's depends_on named: every probe question certified lived-necessary per-question against the LLM-alone leg — Finding 1's §1.1 lesson, the only part of the truncated doc that binds a design). CROSS-CHECK (step 1): no refutation; FROZEN_VS_PLASTIC §10.8 strengthens LG.00 (RT-2 knowledge-loss backs the control clause); one prose id collision found and recorded in the registry comment — DIRECTION_AUDIT.md's "LG.00 = certification" numbering loses to GOAL.md's constitutional citation. DP.04.depends_on += LG.00 same commit, per its own notes. Step 4 (implement + run cheapest) NOT done this iteration: LG.01 is the next unit — CPU, ME.9 PASSES, SmolLM2 is cached on this box. **STILL OWED: the doc's §2.2–§11** (understanding test LG.05, grounding bakeoff arms, ordering experiment) — a research pass with citations, per the empty-queue rule; registering unwritten designs would be the disease coverage.py exists to catch. — **THE OWED RESEARCH PASS IS DONE, 2026-09-04 (builder), and the row's status is now DRAFTED-NOT-REGISTERED.** §2.2–§11 are written: the doc went 178 → ~700 lines with **ten citations fetched and verified this pass** (marked `[V]`: 1910.09451 HIGhER, 2005.07648 LangLfP, 2109.12098 CLIPort, 2112.03227 CALVIN, 1711.00350 SCAN, 2003.05161 gSCAN, 2508.13446 CAST, 2202.01771 Li et al., Bender & Koller ACL 2020, Smith & Yu *Cognition* 2008, Smith & Gasser *Artificial Life* 2005, Needham et al. *IBD* 2002) and one carried `[c]` (2603.19233, not re-verified — the convention this doc already used). **FOUR `Spec(...)` DRAFTS: `LG.03` (cells are language-necessary — fixture, `cpu<10min`), `LG.04` (the grounding bakeoff, five arms, `cpu<2h`), `LG.05` (the Understanding Test, `cpu<2h`), `LG.06` (the ordering experiment, `cpu<2h`).** All four AST-parse, construct against the live `protocol.py`, collide with nothing in `BY_ID`, shadow no prefix, and every `depends_on` resolves — i.e. **step 2 is done; step 3 is deliberately NOT.** THE NEXT UNIT IS REGISTRATION, and it must run step 1 in full — a partial cross-check was done and **already changed two drafts**, which is the argument for doing the rest before registering rather than after: (a) `CAPABILITIES.md` §C L2 carries *"Control: untrained verb must fail"*, absent from `LG.05` and now added — without it held-out cells can be solved by the object alone; (b) `CURIOSITY_BAKEOFF.md`'s F2 branch (*"the fix is hindsight relabeling density, not curiosity"*) names `A4`'s confound, so `C-DENSITY` was added to `LG.04` — relabelling multiplies training pairs and an `A4`-over-`A3` gap is otherwise just more data. No refutation was found. Two constitutional readings are recorded in the doc and must survive registration: the frozen-LLM-embedding arm `A1` runs **SCORED-AND-INELIGIBLE** under SYSTEM.md's 08-24 amendment (the HR.6/A6 precedent from the row above), and `LG.06`'s language-first arm initialises a **plastic** policy from pretrained weights, which PLASTIC-ONLY permits. **RUN `LG.03` FIRST AND ALONE**: it is `cpu<10min`, it is the only spec in the family whose cost is not a guess, and it is designed to be able to **foreclose the whole family cheaply** — if W0 admits no cell where more than one act is plausible from the same observation, the family is VENUE-BLOCKED and that reading is an eleventh instrument for `w0-too-shallow`, bought for ten minutes instead of discovered inside a 2-hour bakeoff. **`T2.16` shares `A4`'s mechanism and has never run** — one implementation, imported by both, not two. — **REGISTERED 2026-09-04 (builder), `a4d9c92`, registry 234→238; steps 1–3 and 5 complete, step 4 NOT done and named below.** THE FULL CROSS-CHECK RAN and it **found two more conflicts the partial pass had missed, one of which was a MANDATORY clause** — which is the whole argument for doing step 1 in the registering iteration rather than trusting a draft's own account of itself. Both are in `FROZEN_VS_PLASTIC.md`, neither is a refutation: **(c)** §10.2 declares **two** controls mandatory for *"every spec that runs with a parent"* and the family carried one. `A0` already is the MUTE-PARENT twin; the **SHUFFLED-PARENT** twin (*"same words, same rate, wrong events… if Jack's grounding survives shuffling, he learned the words' distributional statistics and not their referents"*) had **no analogue anywhere in `LG.03`–`LG.06`**, so **`C-SHUFFLE` was added to `LG.04`** — `A3` trained on mis-paired instructions and trajectories at the same count and the same optimiser steps, which must collapse to `A0`. It is not redundant with `A0`: `A0` has no channel, `C-SHUFFLE` has a channel carrying noise, and an arm whose advantage is the mere PRESENCE of a wider input beats `A0` while tying `C-SHUFFLE`. **(d)** §10.6b's Chaabouni et al. (ACL 2020, 2004.09124) [V] — only 4 of 141 settings correlate test accuracy with any compositionality metric, while generalisation correlates with INPUT SPACE SIZE at ρ = 0.86 — so `LG.05`'s notes now REQUIRE the retained-cell count and vocabulary size in its metrics; a reporting requirement, no bar moved. Mapped and not conflicting: `LG.01` certifies the **Q&A** venue and `LG.03` the **BEHAVIOUR** venue (the certificate `DIRECTION_AUDIT.md`'s *"do not run T2.06/T2.07 before LG.00"* asks for on the behavioural side); `ME.10`'s "held-out compositional pairs" shares a phrase, not a mechanism. WHAT MOVED: `cpu<10min` went EMPTY-and-FILL-HELD → **fillable today (`LG.03`)**; `unreachable` 91→94 and `cpu_foreclosed_now` 36→39, both recorded with the growth-log justification in the same commit. `champions --check` rc=0 and deliberately UNCHANGED — whether `LG.04` becomes the language seat's declared arena is the Review's tie-break DUE 09-07, and an input note now sits on that row. **STEP 4 IS THE NEXT UNIT: implement and run `LG.03`, alone.** It was not started this iteration, and the reason is a venue fact worth inheriting rather than a lack of time: **`LG.03`'s design specifies the two legs but not the W0 verbs, targets or planner**, and `W0` has **no language channel at all** (`w0.py`: *"language 32 ABSENT, and absent as an INPUT CONDITION… W0 has no talker in it yet"*). The object inventory IS there and was measured this iteration (seed 0, 39 geoms): **`apple`, `obj0`–`obj4`, the ladder (`ladder_railL/R` + `rung0`–`rung5`), `platform`, `ramp`, `stair0`–`stair3`, `pool_water`/`pool_floor`, `seesaw_plank`, `welded_block`, `noise_panel`** — comfortably ≥ 4 distinguishable objects, so the ≥4×≥4 grid is not obviously foreclosed on the object axis. **The open design questions, all of them venue questions and none of them answerable by argument:** which verbs the 8-dim rover can actually enact (approach / touch / push / climb are candidates; `climb` exists only for the ladder, which breaks the "every verb represented at least twice" clause unless the platform and stairs count); what the per-cell target predicate is; what the privileged planner is and what "target identity withheld" means to it operationally; and what the per-cell chance band is. Design those in the docstring BEFORE writing the fit, because this is a FIXTURE — every downstream arm is scored against it, and a curated cell set would be undetectable afterwards. |
| DIRECTION_AUDIT.md | WP.01–04, LF.01–05, SO.01–05, PS.07, T0.17–18 (stubs) | **PARTIALLY REGISTERED 2026-09-03 (builder)** — 7 of 17 stubs registered (registry 225→232), 2 REFUSED, 8 HELD, all by the step-1 cross-check; full dispositions in the registry section comment (registry_expansion.py, "DIRECTION_AUDIT.md §5"). REGISTERED: **LF.01** (life runs to natural end; deps T1.06+PS.02, CPU_LONG — refills empty cpu<2h), **LF.02** (save/resume a life; deps T0.03/T0.04/T6.03/ME.1), **SO.01** (third-person stream; dep PG.6; notes corrected at registration — the box NOW renders via GLX/ensure_gl, the stub's osmesa-FAIL numbers predate experiments/render.py), **SO.02** ('I'm cold' is true when he is cold; deps PS.01+VO.01), **SO.04** (being watched does not change him; dep SO.01 — deliberately blocked, UNREACHABLE_BASELINE 90→91 with growth-log justification), **T0.32/T0.33** (the doc's T0.17/T0.18 RENAMED — both ids taken by later harness specs; "id suggestions only" per the doc). REFUSED: **LF.03** (NE.08 owns the cross-life-diary claim — C-WIPE/C-FOREIGN/C-SHUFFLE-TIME subsume the shuffled-diary control; XL.01's power calc binds successors), **LF.05** (T5.08 owns the claim incl. the identical no-learnability-filter control; the stub's own "not both" note). HELD: **WP.01–04, PS.07, LF.04** — W1 world content; the W1 design is the Review's 2026-09-06 (`w0-too-shallow`), same clock as the SURVIVAL_WORLD row below; register with/after the reconciled W.* family. **SO.03, SO.05** — owner-avatar/social world content and a running life harness respectively; the SO-family research pass (row below) must extend ids from SO.06 up, the prefix is now occupied. NAMED GAP for the Review: spectating has NO line in coverage.py's hand-maintained COMMITMENTS despite GOAL.md's "I want to watch him figure out the world himself" — adding it is a coverage.py edit + T0.21/T0.28 re-buys, not smuggled into this registration. Step 4 (implement+run cheapest) NOT done this iteration (LG-row precedent): **T0.32 is the named next unit** — the rtf gate, whose branch LC.07's pilot fired by hand on 09-01. |
...
| OWNER DECISION: the owner's hands (`docs/research/OWNERS_HANDS.md`, written 2026-09-04) | SO.06–SO.09: provisioning channel (drop-in objects), provenance into the diary ("who left this"), anti-puppeteering limits | **REGISTERED 2026-09-04 (builder), steps 1-3 and 5 DONE; step 4 is the named next unit.** Registry 238 -> 242; `SO.06` (`cpu<10min`, deps `XL.00`+`TA.01`, both PASS) is RUNNABLE and `coverage` now reads *fillable today: SO.06* on the first cost class to be fillable since the board emptied; `SO.07`/`SO.08`/`SO.09` are deliberately blocked behind it (`UNREACHABLE_BASELINE` 94 -> 97 in the registering commit, growth-log entry named). **STEP 1 WAS RE-RUN IN FULL AND FOUND ONE REFUTATION THE DRAFTING PASS MISSED, which is the whole reason this protocol re-runs it.** `NE.03`'s notes carry, inherited from `PURPOSE_AND_SCAFFOLDING.md` §5 G-A, the pre-registered rule *energy_accounting_residual != 0 (ERROR, not VOID — the instrument is wrong)* over the identity `Sum_t max(0, delta e_t) == Sum_f nu_f n_f`. `SO.07`'s `C-GIVE` was drafted as *places the item IN CONTACT, **or restores energy directly***; the second half is FORBIDDEN by that rule — a direct write to `e` is a positive delta-e with no logged consumption event, so the residual is non-zero by construction, any run carrying the arm is an ERROR, and the arm could therefore never produce the low-`R` number that is its only job. **STRUCK at registration**, the struck clause recorded on the spec rather than deleted; contact placement survives untouched and is strictly sufficient because consumption still goes through `drives.DriveLayer`'s contact rule. **The consequence is a strengthening and costs nothing:** a hand-placed gift is a food source the identity does not know about, so the first honest drop breaks it — `SO.09` is given the extended identity `Sum_t max(0, delta e_t) == Sum_f nu_f n_f + Sum_hands nu_g n_g`, reconciled against `drives.DriveLayer.ate_total` (already keyed per source), which upgrades an unlogged placement from *unreported* to *arithmetically impossible*. **`SO.08` WAS JUDGED, not waved through** — §3.4 pre-authorised its own refusal against `LG.02` and the judgement is REGISTER, for three recorded reasons: two different GOAL.md sentences (*who LEFT it* vs *whose advice proved TRUE*), an evidence channel whose value is discovered only by eating (delayed, self-administered, on `plants.py`'s dose response — a cautious policy gathers no evidence at all) against `LG.02`'s uttered claim, and an EQUAL-DONORS control leg `LG.02` does not have; the Laplace posterior is IMPORTED, never re-derived. No other refutation: `donor`, `hand_share` and `puppeteering` are unclaimed terms across `docs/research/` and `LESSONS.md`. One venue correction recorded on `SO.06`: `playground.py`'s `probe_objects` uses a `<freejoint/>` and is therefore NOT the contract to copy — `shelters` (geoms only, default empty, byte-identical world when unused) is. **STEP 4 = implement and run `SO.06`, alone**; its hard leg is the bit-identity one (hand present and dropping nothing must be indistinguishable from no-hand, so the PARKED gift must be provably inert). Prior status, kept for provenance: **THE OWED RESEARCH PASS IS DONE, 2026-09-04 (builder); status was DRAFTED-NOT-REGISTERED.** `docs/research/OWNERS_HANDS.md` is new, ~470 lines, with **ten citations fetched and verified this pass** (`[V]`: Caro & Hauser *QRB* 67(2) 1992 — the three-conjunct operational test for teaching; Thornton & McAuliffe *Science* 313(5784):227–229 2006 — wild meerkats provision pups with graded prey and the pups learn faster, the closest published instance of the owner's sentence; Heyes *Biol. Rev.* 69(2):207–231 1994 — the stimulus-enhancement/emulation/imitation partition that names the design target; Csibra & Gergely *TiCS* 13(4):148–153 2009; Lyons, Young & Keil *PNAS* 2007 doi:10.1073/pnas.0704452104 — over-imitation of causally irrelevant actions at 86% vs 16% baseline on first attempt; Wood, Bruner & Ross *JCPP* 17:89–100 1976; Johnson, Hashtroudi & Lindsay *Psych. Bull.* 114(1):3–28 1993 — source monitoring dissociates from content; de Haan, Jayaraman & Levine arXiv:1905.11979 — "access to more information can yield worse performance"; Dennis et al. arXiv:2012.02096 — UED/PAIRED; Ng, Harada & Russell ICML 1999 — PBRS policy invariance) and one carried `[c]` (Co-Reyes et al. arXiv:2006.12478, from `PURPOSE_AND_SCAFFOLDING.md` §1.6). **FOUR `Spec(...)` DRAFTS: `SO.06` (the hand reaches ONLY through the world — fixture, `cpu<10min`), `SO.07` (what the hands leave is FOUND and outlives them, `cpu<2h`), `SO.08` (the diary records WHOSE hands and he acts on it, `cpu<2h`), `SO.09` (a life the hands bought is not evidence — the accountant, `cpu<1min`).** **Step 2 is DONE:** all four AST-parse as one expression, construct against the live `protocol.py`, collide with nothing in `BY_ID`, shadow no prefix, every `depends_on` resolves, and all five distinct deps (`XL.00`, `TA.01`, `PS.01`, `ME.9`, `LG.02`) are **PASS** — so `SO.06` would be runnable on registration and is the only FRESH dispatch available; registry 238→242. **Step 3 is deliberately NOT done** (the `LANGUAGE_GROUNDING.md` precedent: the registering iteration re-runs step 1 in full and finds more). Ids start at `SO.06` because this row's own instruction says the `SO.03`/`SO.05` prefix is occupied. **THE CROSS-CHECK FOUND NO REFUTATION AND FOUR BINDING CONSTRAINTS, all four of which changed a draft:** (a) `PURPOSE_AND_SCAFFOLDING.md` §3.6 already owns the removal metric (`retention_ratio R = C_off/C_on`, `scaffolding_benefit B = C_off/C₀`) so `SO.07` REUSES it verbatim instead of shipping a third definition of "did the help help"; (b) §2.6(ii) proves policy invariance is **vacuous** on W0's zero-reward world (`V^π_shaped = −Φ` for every π), which **deletes** the potential-based-hand arm this doc would otherwise have proposed and forces `SO.09` to be an empirical accountant; (c) `FROZEN_VS_PLASTIC.md` §10.2's two mandatory parent controls apply — the hands are a parent acting through objects — so **`C-MISPLACE`** (same drop rate and item mix, positions/times independent of Jack and of his needs; MUST fail) was added to `SO.07` **before** drafting, which is the `LG.04`/`C-SHUFFLE` lesson applied prospectively; (d) `LG.02` owns attributed trust, so `SO.08` must import its Laplace posterior rather than re-derive one, and **the doc pre-authorises its own REFUSAL** if the registering iteration judges the advice-vs-object gap too thin. **TWO VENUE FACTS measured this pass and recorded so no run has to discover them:** provisioning needs only a `model.body_pos` write on a **static** body (`plants.py`'s idiom — a freejoint is forbidden, it would add dofs to a world whose observation width nine specs depend on), and **W0 already has an undeclared provisioning channel** — food geoms drift and the world never re-places them (`w0.py:_place` omits `mj_resetData` deliberately), which is exactly the "food ratchet" hypothesis `lc03_food_probe.py` was written to chase. Nothing here is W1-held: no avatar, no voice, no language channel. **STEP 3 IS THE NEXT UNIT: run step 1 in full, then register `SO.06`–`SO.09`; then step 4 = implement and run `SO.06`, alone.** Earlier owner note, unchanged: OWNER 2026-08-09: visualisation of needs (hunger bars, spectating) is deliberately LATE — it is a pure read layer, zero science cost. But the camera rolls from day one: NE.* implementation must log needs telemetry (a few floats/step) from the FIRST needful life, so any later viewer can replay any life. Recording is nearly free; retrofitting history is impossible |
