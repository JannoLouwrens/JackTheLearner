...
- [!] **T1.08** Seed variance measured  — backend=kaggle; effect=0.23644
...
- [-] **T6.03** Cross-session persistence  — dependencies not satisfied: T2.10 (FAIL)
...
- [!] **T0.23** A mistyped command cannot spend the GPU budget  — bad_argv_never_dispatched=True; bad_argv_refused=True
...
- [~] **D1.0** Control-path bakeoff: who does motor control (D1's four permitted arms)  — arms={'aprime': {'aux_optimiser_steps': [1472, 1472, 1472], 'consistency': 0.06, 'env_steps': [753664, 753664, 753664], 'final_third_reward': 5.303, 'final_third_slope': 1.74e-07, 'mean': 506.4, 'optimiser_steps': [7360, 7360, 7360], 'paired_t': 20.41, 'ppo_trainable_params': 281763, 'sigma_vs_random': 13.02, 'std': 30.36, 'step_match': 1.005, 'trained_means': [502.6, 538.5, 478.1], 'untrained_means': [196.3, 203.5, 195.4], 'updates_per_env_step': 0.009766}, 'b_split': {'aux_optimiser_steps': [0, 0, 0], 'consistency': 0.056, 'env_steps': [753664, 753664, 753664], 'final_third_reward': 5.228, 'final_third_slope': 4.07e-07, 'mean': 344.0, 'optimiser_steps': [7360, 7360, 7360], 'paired_t': 6.0, 'ppo_trainable_params': 57830650, 'sigma_vs_random': 10.53, 'std': 19.32, 'step_match': 1.005, 'trained_means': [357.2, 321.8, 353.1], 'untrained_means': [101.3, 113.9, 212.9], 'updates_per_env_step': 0.009766}, 'c_e2e': {'aux_optimiser_steps': [0, 0, 0], 'consistency': 0.057, 'env_steps': [753664, 753664, 753664], 'final_third_reward': 5.211, 'final_third_slope': -8.6e-08, 'mean': 350.0, 'optimiser_steps': [7360, 7360, 7360], 'paired_t': 9.25, 'ppo_trainable_params': 57363705, 'sigma_vs_random': 10.8, 'std': 20.12, 'step_match': 1.005, 'trained_means': [330.8, 370.9, 348.3], 'untrained_means': [136.4, 142.2, 191.9], 'updates_per_env_step': 0.009766}, 'd_mlp': {'aux_optimiser_steps': [0, 0, 0], 'consistency': 0.056, 'env_steps': [753664, 753664, 753664], 'final_third_reward': 5.411, 'final_third_slope': 3.01e-07, 'mean': 415.0, 'optimiser_steps': [7360, 7360, 7360], 'paired_t': 16.96, 'ppo_trainable_params': 530339, 'sigma_vs_random': 12.99, 'std': 23.41, 'step_match': 1.005, 'trained_means': [426.3, 430.7, 388.1], 'untrained_means': [196.1, 200.3, 196.3], 'updates_per_env_step': 0.009766}}; kernels=[{'arms': ['aprime', 'd_mlp'], 'backend': 'kaggle', 'gpu': 'Tesla P100-PCIE-16GB', 'head': '1ee4a3a', 'wall_minutes': 241.2}, {'arms': ['b_split'], 'backend': 'kaggle', 'gpu': 'Tesla P100-PCIE-16GB', 'head': '321d786', 'wall_minutes': 357.0}, {'arms': ['c_e2e'], 'backend': 'kaggle', 'gpu': 'Tesla P100-PCIE-16GB', 'head': 'ce2b56b', 'wall_minutes': 356.1}]
...
- [!] **PS.06** Tiring is a price: sustained exertion drains the body's capability before anyone has to learn it  — borrow_ok=1.0; borrow_ok_std=0.0
...
- [!] **PS.08** Heavy is a price: moving mass costs need-currency before anyone has to learn it  — borrow_ok=1.0; borrow_ok_std=0.0
      - _asserts:_ The venue charges for moving mass, in the currency the needs already speak: a scripted transport policy driven to displace playground objects of >= 4 pre-registered masses (inside the EXISTING object_mass_range 0.2-3.0 kg, playground.py:123 — no new world content) over an identical displacement pays a need-cost (integrated drive drain read off the existing needs.py channels: M = M_BASAL + kappa_act*p_mech, needs.py:245, with fatigue riding the same p_mean, needs.py:513 — no new instrument) that increases MONOTONICALLY in object mass on every seed; the light-vs-heavy cost gap exceeds the venue's outcome-metric quantum (the DP.04/W1.02 resolution lesson applied at registration, not discovered by a pilot); and the load is LEGIBLE beforehand — a probe on the sensory vector predicts the trip's coming cost (or the mass class) above chance early in the push, while there is still time to decline the load (PS.02's legibility gate, reused because it is the same question).
      - _dies if:_ Cost flat or non-monotone in mass (moving mass is effectively free — friction and inertia never reach the needs accounting at this body's scales); or the light-vs-heavy gap reads below the outcome quantum, so no learner could ever be selected on load; or the probe cannot beat chance, so heavy is a price paid blind. Any of the three means 'heavy' is not learnable in this venue and GOAL.md:187's promise has no mechanism behind it.
      - _then delete:_ On FAIL: 'heavy' is struck from what this venue can teach, every carrying, hauling or load-budgeting claim is inadmissible here, and the repair is a world/body edit routed through the W1 window — never a quiet friction or mass-range retune until the price appears.
- [~] **PS.09** Worth-it is a real distinction: the world pays and charges in one currency, and some trips do not pay  — ate_ok=1.0; ate_ok_std=0.0
      - _asserts:_ W0's ledger makes net value computable and NON-VACUOUS: resource payoffs and traversal prices are the SAME need-currency (floor food restores nu = 0.08, drives.py:68, while PS.05 MEASURED the traversal price at 0.016-0.069 over 1.5-6 m — the payoff sits INSIDE the price span, so a break-even exists in-world at registration time), and across >= 4 pre-registered (payoff, distance) offers straddling that break-even, a scripted collect-trip's NET need-gain (restored minus drained, both read off the existing needs.py channels — no new instrument) is POSITIVE for at least one registered offer and NEGATIVE for at least one, on every seed, each side's margin exceeding the venue's outcome-metric quantum; and the sign is LEGIBLE beforehand — a probe on the PRE-DEPARTURE sensory vector predicts the trip's net sign above chance before the first step (PS.02's legibility gate, reused because it is the same question).
      - _dies if:_ Every reachable offer is net-positive (there is nothing to decline, so 'worth-it' is a distinction the world never asks for); or every offer is net-negative (nothing is worth doing and foraging is a losing game at every distance); or the margins at the straddle read below the outcome quantum, so no learner could ever be selected on declining; or the probe cannot beat chance, so worth-it is decided blind. Any of the four means 'worth-it' is not learnable in this venue and GOAL.md:187's promise has no mechanism behind it.
      - _then delete:_ On FAIL: 'worth-it' is struck from what W0 can teach, every cost-benefit, declining or foraging-selectivity claim is inadmissible in this venue, and the repair is a world edit routed through the W1 window (a payoff schedule and a price curve that actually cross) — never a quiet nu or decay-rate retune until the trade appears.
...
- [!] **LG.12** He speaks correctly or he is silent — the abstaining mouth  — gate_rejected_fab_frac=1.0; gate_rejected_fab_frac_std=0.0
...
- [~] **BA.03** He braces against a surface — balance is used where direction still has authority  — best_trained=11.867778; best_trained_std=0.0734637
      - _asserts:_ In a scenario where a graspable surface is within reach, a learner given BA.01's vestibular channel PLACES ITS SUPPORT ON THE LEAN SIDE and holds himself measurably more upright — LOWER integrated absolute tilt over the fixed 12 s window, the 2026-09-20 Review FULL ruling's statistic (option (c), `ba03-null-saturates-the-horizon`; time-to-topple saturated: the blind twin held 98.9% of the horizon) — than an identical learner trained with the channel deleted (>= 3 sigma across seeds), and the gain vanishes when the channel is replaced by matched-statistics noise.
      - _dies if:_ No tilt gain from having the channel even with a surface in reach. Then D8's open-ground finding generalises — balance is decoded and never acted on in ANY scenario this body affords — and the honest status of balance-as-a-used-sense is 'sensed, unused' until the playground humanoid exists. A SECOND honest outcome: the sensing arm wins but the brace lands on the wrong side as often as the right one, which refutes the mechanism while the number passes; brace-side accuracy is therefore a REPORTED gate, not a footnote.
...
- [x] **LT.02** The self-generated-chaos detector works (PG.4's blind spot)
      - _asserts:_ A curiosity agent can farm BODY-CARRIED irreducible surprise with zero noise-panel dwell, and the chaos detector sees it: ragdoll-ICM-noise (panel deleted, adhesion 0, genuinely stochastic actuation noise executed while the commanded action is logged — attempt 2's Review-ruled true positive, 2026-09-25) scores chaos_occupancy >= 3.0 while PG.4's dwell metric reads 0.000; the NOISE-FREE farmer still satisfies chaos_reward_ratio >= 2.0 and dwell 0.000 (attempt 1's passing controls, kept); and the scripted climber — which moves hard and falls repeatedly — scores chaos_occupancy <= 1.0. GUARD: attempt 1 measured noise-free body chaos REDUCIBLE (occupancy 0.145 vs 3.0); that stays falsified, is re-reported every run, and a PASS certifies the DETECTOR, never that his own body is a noise trap.
      - _dies if:_ Ragdoll-ICM-noise — a by-construction irreducible body-carried source — is NOT flagged (the detector is blind and no arm's immunity may be reported), or the scripted climber IS flagged (the detector penalises coordinated motion and falling, i.e. the behaviour GOAL.md asks for).
